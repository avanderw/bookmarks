...
export function filterTags(tagSummary: TagInfo[], searchText: string): TagInfo[] {
	...
	return tagSummary.filter((tagInfo) => tagInfo.tag.includes(searchLower));
}
...
export function getBookmarksByTags(bookmarks: Bookmark[], tags: string[]): Bookmark[] {
	...
	return bookmarks.filter((bookmark) => {
		...
		return bookmark.tags.some((bookmarkTag) => normalizedTags.includes(bookmarkTag.toLowerCase()));
	});
}
