...
function normalizeUrl(url: string): string {
  try {
    ...
    let normalized = urlObj.hostname.replace(/^www\./, '') + urlObj.pathname + urlObj.search;
...
function findExactUrlDuplicates(bookmarks: Bookmark[]): DuplicateGroup[] {
  ...
  return Array.from(urlMap.entries())
    ...
    .map(([url, bookmarks], index) => ({
      ...
      reason: `Same URL (${url})`,
      bookmarks: bookmarks.sort((a, b) => new Date(a.added).getTime() - new Date(b.added).getTime()),
      confidence: 1.0
    }));
}
    ...
    domainBookmarks.forEach((bookmark1, i) => {
      ...
      if (similarBookmarks.length > 1) {
        ...
        groups.push({
          ...
          bookmarks: similarBookmarks.sort((a, b) => new Date(a.added).getTime() - new Date(b.added).getTime()),
          confidence: 0.8
        });
      }
    });
...
export function getTypeLabel(type: DuplicateGroup['type']): string {
  switch (type) {
    case 'exact_url': return 'Exact Duplicate';
