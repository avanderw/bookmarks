...
function checkStorageSize(dataSize: number, totalSize: number): void {
    ...
    const bookmarkCount = JSON.parse(localStorage.getItem(STORAGE_KEY) || '{"bookmarks":[]}').bookmarks?.length || 0;
    ...
    if (totalSize > CRITICAL_THRESHOLD) {
        ...
        if (window.confirm(
            ...
            `Your ${bookmarkCount} bookmarks are using ${(dataSize / 1024 / 1024).toFixed(1)}MB.\n\n` +
            `Would you like to:\n` +
            ...
            // Trigger export
            const event = new CustomEvent('storage-full-export', { detail: { dataSize, totalSize, bookmarkCount } });
...
export async function importBookmarks(file: File): Promise<BookmarkStore> {
    ...
    try {
        ...
        throw new Error(`Failed to import bookmarks: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
