...
function getLocalStorage(): BookmarkStore {
...
const sync = writable({
	available: browser && 'showOpenFilePicker' in window,
	running: false,
	changes: false
});
...
async function watchFileSystem() {
	...
	if (!fileSystem.handle || !fileSystem.cache.file) {
		// not initialized
		return;
	}

	const updateFile = await fileSystem.handle.getFile();
	if (updateFile.lastModified === fileSystem.cache.file.lastModified) {
		// no change, don't read file
	...
	if (updateHash === fileSystem.cache.hash) {
		// modified, same as current
		watchFileSystem();
		return;
	}
	fileSystem.cache.hash = updateHash; // otherwise, update hash

	if (updateHash === cacheHash) {
		// different from current, same as staging
		watchFileSystem();
		return;
	}
...
export function downloadCache() {
	...
	a.href =
		'data:text/plain;charset=utf-8,' + encodeURIComponent(JSON.stringify(cacheObject, null, 2));
	...
	if (data && data.bookmarks && Array.isArray(data.bookmarks)) {
		...
		if (removedCount > 0) {
			...
			if (browser && window.alert) {
				window.alert(
					`Import complete. ${removedCount} bookmark(s) with invalid URLs were removed.`
				);
			}
		}
	}
